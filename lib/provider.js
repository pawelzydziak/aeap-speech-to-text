...
// const DEFAULT_MODEL = [{phrases:[$OPERAND]}];
...
class GoogleProvider extends Writable {
	...
	// static speechCont = [{phrases:[$OPERAND]}]
	/**
	...
	constructor(options) {
		...
		this.config = {
			...
			speechContexts: [{phrases: ['$OPERAND']}],
		};
	...
	setConfig(config) {
		...
		if (config.codec) {
			if (!(config.codec.name in GoogleProvider.encodings)) {
				throw new Error("Codec '" + config.codec.name + " 'not supportedp");
			}
		...
		if (config.language) {
			if (!GoogleProvider.languages.includes(config.language)) {
				throw new Error("Language '" + config.language + " 'not supportedpr");
			}
